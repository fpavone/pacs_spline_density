smoothingSplines <- function(k,l,alpha,data,xcp,knots,num_points = 100) 
{
  ...
  if( length(knots) == 1 )
  {
    ...
    obj <- .Call("mymain",as.integer(k),as.integer(l),alpha,data,xcp,knots_,as.integer(num_points))
  }
  else 
   obj <- .Call("mymain",as.integer(k),as.integer(l),alpha,data,xcp,knots,as.integer(num_points))
...
plot.smoothSpl <- function(obj, by = 1 , n = 10,...){
  ...
  plot.default(xx,obj$Y_clr[1,],ylim=c(min(obj$Numbers),max(obj$Numbers)), 
               type = "l", xlab = "", ylab = "")
  title("Smoothing splines in clr-transformed space")
  for(i in seq(1,n,by=by)){
    lines(xx,obj$Y_clr[i,], col = cols[i%%length(cols)])
  }
  
  # Plotting densities in orginal space
  plot.default(xx,obj$Y[1,],ylim=c(min(obj$Y),max(obj$Y)), 
               type = "l", xlab = "", ylab = "")
  title("Density")
  for(i in seq(1,n,by=by)){
